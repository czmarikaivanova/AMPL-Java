...
The communication network is represented by a connected undirected unweighted graph $G=(V,E)$ and a subset $S\subseteq V$ referred to as the set of sources, where $|V|=n$. 
...
In the following, we use $d$-step Fibonacci numbers \cite{noe05}, a generalization of well known (2-step) Fibonacci numbers, defined by letting, 
$F^{(d)}_k=0$ for $k\leq 0$, $F^{(d)}_1=1$, and 
other terms according to linear recurrence relation 
\begin{align*}
F^{(d)}_k &=\sum\limits_{i=1}^d F^{(d)}_{k-i}, &\text{ for } k\geq 2.
\end{align*}
...
The maximum number of nodes informed within $t$ time steps is achieved if nodes become informad in the order of their decreasing degree.
\end{lemma}
\begin{proof}
A node with degree $d_i$ informed in $i$-th time step can inform at most $\min\{d_i-1,t-1\}$ nodes.
...
$$
If the order in which $v_k$ and $v_l$ are informed is swithced, i.e. when $v_k$ and $v_\ell$ is informed in the $\ell$-th and $k$-th time step, respectively, 
...
In each iteration $k$, Alg. \ref{alg:dreg} degrees in $T^k$ of maximum possible number of nodes are increased in the order of their decreasing degree.
